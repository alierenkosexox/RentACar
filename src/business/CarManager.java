...
public class CarManager {
    ...
    public ArrayList<Object[]> getForTable(int size, ArrayList<Car> cars) {
        ...
        Iterator var4 = cars.iterator();

        while(var4.hasNext()) {
            Car obj = (Car)var4.next();
            int i = 0;
    ...
    public ArrayList<Car> searchForBooking(String strt_date, String fnsh_date, Model.Type type, Model.Gear gear, Model.Fuel fuel) {
        ...
        Iterator var17 = bookList.iterator();

        while(var17.hasNext()) {
            Book book = (Book)var17.next();
            busyCarId.add(book.getCar_id());
        }

        searchedCarList.removeIf((car) -> {
            return busyCarId.contains(car.getId());
        });
